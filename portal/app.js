...
function renderHomePlayback(force=false) {
  ...
  const otherZones=otherPlaying.length?`<button class="featured-other-zones" data-go="playback">${icon("speaker_group")}También suena en ${esc(otherPlaying[0].display_name)}${otherPlaying.length>1?` y ${otherPlaying.length-1} más`:""}</button>`:"";
  $("#home-hero").classList.remove("skeleton-block"); $("#home-hero").innerHTML = featured ? `<div class="featured-backdrop">${imageTag(featured.now_playing?.image_key,"",1200)}</div><div class="featured-cover">${cover(featured.now_playing?.image_key,np.line1)}</div><div class="featured-content"><span class="overline">Ahora suena · ${esc(featured.display_name)}</span><h1>${esc(np.line1 || "Música en Roon")}</h1><p class="featured-artist">${esc(np.line2 || "")}</p><p class="featured-album">${esc(np.line3 || "")}</p>${otherZones}<div class="featured-controls" data-zone="${esc(featured.zone_id)}"><button class="icon-btn" data-zone-command="previous" title="Anterior">${icon("skip_previous")}</button><button class="play-main" data-zone-command="playpause" title="${featured.state === "playing" ? "Pausa" : "Reproducir"}">${icon(featured.state === "playing" ? "pause" : "play_arrow")}</button><button class="icon-btn" data-zone-command="next" title="Siguiente">${icon("skip_next")}</button><button class="btn text" data-go="playback">Abrir zona ${icon("arrow_forward")}</button></div></div><div class="featured-meta"><span>${online ? esc(dashboard?.status?.core_name || "Roon Core") : "Core desconectado"}</span><span>${dashboard?.counts?.playlists||0} playlists · ${dashboard?.counts?.playlist_tracks||0} pistas</span></div>` : `<div class="featured-content empty-feature"><span class="overline">${online ? "Roon está preparado" : "Core sin conexión"}</span><h1>${greeting()}</h1><p>Busca un álbum, artista o canción para empezar a escuchar.</p><button class="btn primary" data-quick="music:search">${icon("search")}Buscar música</button></div>`;
...
async function searchMusic(query) {
  ...
  recordHomeHistory({event_type:"search",title:query.trim(),subtitle:"Búsqueda de música",query:query.trim()});
}
