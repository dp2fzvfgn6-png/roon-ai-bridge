...
const state = {
  ...
  playlists: [], temporaryPlaylists: [], selectedPlaylist: null, search: null, keys: [], users: [], tools: [], presets: [], connections: null,
  sessionUser: null, debugMode: false, installedChannel: "stable",
  ...
  miniRenderSignature: null, homePlaybackSignature: null, homePreviewZoneId: null, homePreviewTimer: null, homeIdleTimer: null, homeIdleMosaicCleanup: null,
  homeHistory: { play: [], search: [], totals: { play: 0, search: 0 }, loading: null },
  playlistSort: localStorage.getItem("roonia.portal.playlist-sort") || "recent", playlistReorder: null,
...
function setRangeFill(input){const min=Number(input.min)||0;const max=Number(input.max)||0;const value=Number(input.value)||0;const percent=max>min?Math.max(0,Math.min(100,((value-min)/(max-min))*100)):0;input.style.setProperty('--range-progress',`${percent}%`);}
function syncMiniRangeFills(root=document){$$('[data-mini-seek],[data-mini-volume]',root).forEach(setRangeFill);}
...
const portalObserver = new MutationObserver((records) => {
  ...
  roots.forEach((root) => { hydrateImages(root); syncMiniRangeFills(root); });
});
...
const ACTION_MESSAGES = {
  ...
  volume_step_output: ({ output, direction }) => `${output}: volumen ${direction > 0 ? "subido" : "bajado"} un paso`,
  volume_step_group: ({ zone, count, direction }) => `${zone}: volumen ${direction > 0 ? "subido" : "bajado"} un paso en ${count} ${count === 1 ? "salida" : "salidas"}`,
...
function closeModal() { closePlaybackActions(); closeContextModal(); if ($("#modal").open) $("#modal").close(); $("#modal-content").innerHTML = ""; state.modalSearch = null; state.playlistReorder = null; }
...
function playlistModalHead(overline) { return `<div class="modal-head playlist-modal-head"><span class="overline">${esc(overline)}</span><button class="icon-btn" data-close>${icon("close")}</button></div>`; }
...
function renderAvailableUpdateNotice(update){const notice=$("#header-update-notice");const available=Boolean(update?.version);const beta=update?.channel==='beta';notice.hidden=!available;notice.textContent=beta?'Nueva actualización beta disponible':'Nueva actualización disponible';notice.title=available?`Actualizar a v${update.version}${beta?' (beta)':''}${update.build?` · build ${update.build}`:''}`:'';}
function renderPortalVersion(version,channel){$("#version-badge").textContent=`v${version||"—"}${channel==='beta'?' (beta)':''}`;}
function applyDebugMode(enabled){state.debugMode=enabled===true;if(!state.debugMode)state.temporaryPlaylists=[];$$('[data-debug-only]').forEach((node)=>{node.hidden=!state.debugMode;});if(state.view==='music'&&state.musicTab==='playlists')renderPlaylists();}
function applySession(session){state.sessionUser=session.user||null;state.installedChannel=session.installed_channel||'stable';$("#rail-username").textContent=session.user?.username||"Administrador";applyDebugMode(session.debug_mode);renderPortalVersion(session.version,state.installedChannel);renderAvailableUpdateNotice(session.available_update);}
...
async function loadHome() {
  ...
  state.homeHistory.play=dashboard.recent_listening_history||[];state.homeHistory.search=dashboard.recent_search_history||[];state.homeHistory.totals=dashboard.history_totals||{play:0,search:0};
...
function fitFeaturedTitle(){const copy=$("#home-hero .featured-track-copy");const slot=$("#home-hero .featured-title-slot");const title=$("#home-hero .featured-title");if(!copy||!slot||!title)return;title.style.fontSize="";let size=Number.parseFloat(getComputedStyle(title).fontSize)||82;const minimum=window.innerWidth<=780?18:32;while(size>minimum&&(copy.scrollHeight>copy.clientHeight+1||title.scrollWidth>slot.clientWidth+1)){size-=1;title.style.fontSize=`${size}px`;}}
...
function scheduleHomePreviewReturn(zoneId){if(state.homePreviewTimer)clearTimeout(state.homePreviewTimer);state.homePreviewTimer=setTimeout(()=>{if(state.homePreviewZoneId!==zoneId||state.zones.some((item)=>item.state==="playing"))return;state.homePreviewZoneId=null;state.homePreviewTimer=null;state.homePlaybackSignature=null;renderHomePlayback(true);},5000);}
const HOME_IDLE_GRACE_MS=2600;
function cancelHomeIdleTransition(){if(state.homeIdleTimer)clearTimeout(state.homeIdleTimer);state.homeIdleTimer=null;}
function scheduleHomeIdleTransition(){if(state.homeIdleTimer)return;state.homeIdleTimer=setTimeout(()=>{state.homeIdleTimer=null;if(state.view!=="home"||state.homePreviewZoneId||state.zones.some((zone)=>zone.state==="playing"))return;state.homePlaybackSignature=null;renderHomePlayback(true);},HOME_IDLE_GRACE_MS);}
...
function homeIdleArtworkShare(count,index){
  if(count<=1)return 1;
  const layouts=[homeIdleLayoutEdges(count),...Array.from({length:count},(_,activeIndex)=>homeIdleLayoutEdges(count,activeIndex))];
  const center=(index+.5)/count;const minimum=Math.min(...layouts.map((edges)=>edges[index]));const maximum=Math.max(...layouts.map((edges)=>edges[index+1]));
  return Math.min(1.2,2*Math.max(center-minimum,maximum-center)+.08);
}
function homeIdleImageRequestSize(cssPixels,pixelRatio=1){const density=Math.min(1.75,Math.max(1,Number(pixelRatio)||1));const required=Math.ceil(Math.max(1,cssPixels)*density);return Math.min(1600,Math.max(512,Math.ceil(required/128)*128));}
function stopHomeIdleMosaic(){if(state.homeIdleMosaicCleanup)state.homeIdleMosaicCleanup();state.homeIdleMosaicCleanup=null;}
function transitionHomeHero(root,mode,render){const previousMode=root.dataset.mode;const animate=Boolean(previousMode&&previousMode!==mode&&!window.matchMedia("(prefers-reduced-motion: reduce)").matches);const ghost=animate?root.cloneNode(true):null;if(ghost){ghost.removeAttribute("id");ghost.querySelectorAll("[id]").forEach((node)=>node.removeAttribute("id"));ghost.classList.remove("skeleton-block","home-hero-mode-enter");ghost.classList.add("home-hero-transition-ghost");ghost.setAttribute("aria-hidden","true");ghost.inert=true;}render();root.dataset.mode=mode;if(!animate)return;root.classList.remove("home-hero-mode-enter");void root.offsetWidth;root.classList.add("home-hero-mode-enter");root.append(ghost);requestAnimationFrame(()=>ghost.classList.add("is-leaving"));setTimeout(()=>{ghost.remove();root.classList.remove("home-hero-mode-enter");},520);}
...
function initHomeIdleMosaic(root){
  ...
  function updateArtworkGeometry(){const width=Math.max(1,cloud.clientWidth);const height=Math.max(1,cloud.clientHeight);zones.forEach((zone,index)=>{const art=$(".home-idle-art",zone);const cover=$(".cover",art);const image=$("img[data-image-key]",art);if(!cover)return;const size=Math.ceil(Math.max(height,width*homeIdleArtworkShare(count,index)));cover.style.left=`${((index+.5)/count)*100}%`;cover.style.width=`${size}px`;cover.style.height=`${size}px`;if(image&&!image.dataset.imageLoading)image.dataset.imageSize=String(homeIdleImageRequestSize(size,window.devicePixelRatio));});}
  zones.forEach((zone,index)=>{zone.addEventListener("pointerenter",()=>setActive(index));zone.addEventListener("focus",()=>setActive(index));});
  updateArtworkGeometry();zones.forEach(applyHomeIdleArtworkContrast);
  function draw(time,immediate=false){
    ...
    const spacing=1/count;const driftAmplitude=motion.matches?0:Math.min(.018,spacing*.08);const angleAmplitude=motion.matches?0:Math.min(.035,spacing*.14);const ambientTime=time*1.2;const boundaries=edges.map((edge,index)=>{if(index===0)return {top:0,bottom:0};if(index===count)return {top:1,bottom:1};const phase=index*1.73;const drift=Math.sin(ambientTime/(8800+index*650)+phase)*driftAmplitude;const angle=Math.sin(ambientTime/(7200+index*520)+phase*.67)*angleAmplitude;return {top:edge+drift+angle,bottom:edge+drift-angle};});
    const cloudWidth=Math.max(1,cloud.clientWidth);const compact=cloudWidth<=780;zones.forEach((zone,index)=>{const left=boundaries[index];const right=boundaries[index+1];zone.style.clipPath=`polygon(${left.top*100}% 0,${right.top*100}% 0,${right.bottom*100}% 100%,${left.bottom*100}% 100%)`;const tag=$(".home-idle-zone-tag",zone);const track=$(".home-idle-track",zone);const tagLeft=left.top+(left.bottom-left.top)*.27;const tagRight=right.top+(right.bottom-right.top)*.27;const tagInset=compact?10:18;if(tag){tag.style.left=`${tagLeft*cloudWidth+tagInset}px`;tag.style.maxWidth=`${Math.max(0,(tagRight-tagLeft)*cloudWidth-tagInset*2)}px`;}const trackLeft=left.top+(left.bottom-left.top)*.84;const trackRight=right.top+(right.bottom-right.top)*.84;const trackInset=compact?12:22;if(track){track.style.left=`${trackLeft*cloudWidth+trackInset}px`;track.style.width=`${Math.max(0,(trackRight-trackLeft)*cloudWidth-trackInset*2)}px`;}zone.classList.toggle("is-expanded",index===activeIndex);});
  }
  ...
  const resizeObserver=typeof ResizeObserver==="function"?new ResizeObserver(()=>{updateArtworkGeometry();draw(performance.now(),true);}):null;resizeObserver?.observe(cloud);
  ...
  state.homeIdleMosaicCleanup=()=>{stopped=true;if(frame)cancelAnimationFrame(frame);resizeObserver?.disconnect();motion.removeEventListener?.("change",handleMotionChange);};
}
function renderIdleHomeHero(online){
  const root=$("#home-hero");
  transitionHomeHero(root,"idle",()=>{root.classList.add("home-hero-idle");root.innerHTML=`<div class="home-idle-ambient"></div><div class="home-idle-shell"><div class="home-idle-copy"><span class="overline">Tus zonas están en pausa</span><h1>Elige dónde seguir escuchando</h1><p>Selecciona una carátula para recuperar sus controles.</p></div><div class="home-idle-cloud">${state.zones.map((zone)=>{const np=zone.now_playing||{};const song=np.line1||"Nada en reproducción";const artist=np.line2||"Roon";return `<button type="button" class="home-idle-zone" data-preview-zone="${esc(zone.zone_id)}" aria-label="Abrir ${esc(zone.display_name)}: ${esc(song)}"><div class="home-idle-art"><div class="cover" data-fallback-kind="track"><div class="cover-fallback track">${icon("music_note")}</div>${imageTag(np.image_key,np.line1,1024)}</div><span class="home-idle-zone-tag">${esc(zone.display_name)}</span><span class="home-idle-track"><strong>${esc(song)}</strong><small>${esc(artist)}</small></span></div></button>`;}).join("")}</div></div><div class="featured-meta"><span>${online?esc(state.dashboard?.status?.core_name||"Roon Core"):"Core desconectado"}</span><span>${state.dashboard?.counts?.playlists||0} playlists · ${state.dashboard?.counts?.playlist_tracks||0} pistas</span></div>`;});
...
function beginHomeZonePreview(zoneId){
  ...
  state.activeZoneId=zoneId;localStorage.setItem("roonia.portal.active-zone",zoneId);state.homePreviewZoneId=zoneId;state.homePlaybackSignature=null;renderMiniPlayer();renderHomePlayback(true);
  scheduleHomePreviewReturn(zoneId);
  notifyAction("active_zone",{zone:zone.display_name});
}
function renderHomePlayback(force=false) {
  const dashboard=state.dashboard;const playingZones=state.zones.filter((zone)=>zone.state==="playing");
  if(playingZones.length){cancelHomeIdleTransition();if(state.homePreviewZoneId)clearHomePreview();}
  const signature=JSON.stringify({active:activeZone()?.zone_id||null,preview:state.homePreviewZoneId,zones:state.zones.map(playbackSignature)});
  ...
  if(!playingZones.length&&!state.homePreviewZoneId&&state.zones.length){const root=$("#home-hero");if(!force&&root.dataset.mode==="playback"){scheduleHomeIdleTransition();renderHomeNow();return;}cancelHomeIdleTransition();root.classList.remove("skeleton-block");renderIdleHomeHero(online);renderHomeNow();return;}
  stopHomeIdleMosaic();
  ...
  const root=$("#home-hero");transitionHomeHero(root,"playback",()=>{root.classList.remove("skeleton-block","home-hero-idle");root.innerHTML = featured ? `<div class="featured-backdrop">${imageTag(featured.now_playing?.image_key,"",500)}</div><div class="featured-cover">${cover(featured.now_playing?.image_key,np.line1)}</div><div class="featured-content"><span class="overline">${isPreview?"En pausa":"Ahora suena"} · ${esc(featured.display_name)}</span><div class="featured-track-copy"><div class="featured-title-slot"><h1 class="featured-title">${esc(np.line1 || "Música en Roon")}</h1></div><p class="featured-artist">${esc(np.line2 || "")}</p><p class="featured-album">${esc(np.line3 || "")}</p></div>${otherZones}<div class="featured-controls" data-zone="${esc(featured.zone_id)}"><button class="icon-btn" data-zone-command="previous" title="Anterior">${icon("skip_previous")}</button><button class="play-main" data-zone-command="playpause" title="${featured.state === "playing" ? "Pausa" : "Reproducir"}">${icon(featured.state === "playing" ? "pause" : "play_arrow")}</button><button class="icon-btn" data-zone-command="next" title="Siguiente">${icon("skip_next")}</button><button class="btn text" data-go="playback">Abrir zona ${icon("arrow_forward")}</button></div></div><div class="featured-meta"><span>${online ? esc(dashboard?.status?.core_name || "Roon Core") : "Core desconectado"}</span><span>${dashboard?.counts?.playlists||0} playlists · ${dashboard?.counts?.playlist_tracks||0} pistas</span></div>` : `<div class="featured-content empty-feature"><span class="overline">${online ? "Roon está preparado" : "Core sin conexión"}</span><h1>${greeting()}</h1><p>Busca un álbum, artista o canción para empezar a escuchar.</p><button class="btn primary" data-quick="music:search">${icon("search")}Buscar música</button></div>`;});
...
const HOME_HISTORY_PAGE_SIZE=10;
function listeningHistoryRow(entry){const content=`${cover(entry.image_key,entry.title,"","track")}<span class="home-history-copy"><strong>${esc(entry.title)}</strong><small>${esc(entry.subtitle||"Artista desconocido")}</small><small class="history-zone">${icon("speaker")}${esc(entry.zone_name||"Zona desconocida")}</small></span><time>${esc(fmtDate(entry.created_at))}</time>`;return entry.result_id?`<button class="home-history-row listening-history-row" data-home-history="${esc(entry.history_id)}">${content}</button>`:`<article class="home-history-row listening-history-row">${content}</article>`;}
function searchHistoryRow(entry){return `<button class="home-history-row search-history-row" data-home-history="${esc(entry.history_id)}"><span class="home-history-icon">${icon("search")}</span><strong>${esc(entry.query||entry.title)}</strong><time>${esc(fmtDate(entry.created_at))}</time></button>`;}
function renderHomeHistory(){const listening=state.homeHistory.play;const searches=state.homeHistory.search;$("#home-listening-history").innerHTML=listening.length?listening.map(listeningHistoryRow).join(""):empty("Aún no hay escuchas","Las canciones aparecerán cuando empiecen a sonar en una zona.","headphones");$("#home-search-history").innerHTML=searches.length?searches.map(searchHistoryRow).join(""):empty("Aún no hay consultas","Tus búsquedas musicales aparecerán aquí.","search");const playMore=$("#more-listening-history");const searchMore=$("#more-search-history");playMore.hidden=listening.length>=state.homeHistory.totals.play;searchMore.hidden=searches.length>=state.homeHistory.totals.search;playMore.disabled=state.homeHistory.loading==="play";searchMore.disabled=state.homeHistory.loading==="search";}
async function loadMoreHomeHistory(type){if(state.homeHistory.loading)return;state.homeHistory.loading=type;renderHomeHistory();try{const entries=state.homeHistory[type];const payload=await api(`/api/history?type=${encodeURIComponent(type)}&limit=${HOME_HISTORY_PAGE_SIZE}&offset=${entries.length}`);state.homeHistory[type]=entries.concat(payload.entries||[]);state.homeHistory.totals[type]=payload.total||0;}finally{state.homeHistory.loading=null;renderHomeHistory();}}
...
async function openMedia(resultId, { playlistId = null } = {}) {
  ...
  if (item.media_type === "artist") {
    ...
    const catalogSections=(detail.release_sections||[]).filter((section)=>section.releases?.length).map((section)=>releaseSection(section.title,section.releases,playlistId)).join("");
    const fallbackSections=`${releaseSection(albumTitle,detail.albums,playlistId)}${releaseSection("EPs",eps,playlistId)}${releaseSection("Singles",singles,playlistId)}${releaseSection("Singles y EPs",mixed,playlistId)}`;
    const sections = `${popular.length ? `<div class="modal-section"><div class="entity-section-head"><div><span class="overline">Resultados relacionados de Roon</span><h3>Canciones destacadas</h3></div></div>${resultTracks(popular,playlistId)}</div>` : ""}${catalogSections||fallbackSections}`;
...
function closePlaybackActions() { const popover=$("#playback-actions-popover");if(!popover)return;try{if(popover.matches(':popover-open'))popover.hidePopover();}catch{}popover.hidden=true;popover.innerHTML=""; }
function positionPlaybackActions(anchor) { const popover=$("#playback-actions-popover");if(!popover||popover.hidden)return;const rect=anchor.getBoundingClientRect();const margin=8;const width=popover.offsetWidth;const height=popover.offsetHeight;const left=Math.max(margin,Math.min(window.innerWidth-width-margin,rect.right-width));const below=rect.bottom+margin;const top=below+height<=window.innerHeight-margin?below:Math.max(margin,rect.top-height-margin);popover.style.left=`${left}px`;popover.style.top=`${top}px`; }
function openPlaybackActions(anchor, { kind = "media", resultId = null, playlistId = null, trackId = null, title = "Esta canción" } = {}) {
  const zone = syncActiveZone();
  if (!zone) return toast("Selecciona una zona antes de reproducir", "error");
  const popover=$("#playback-actions-popover");
  const modes=kind==="media"?["replace_queue","play_next","append"]:["play_now","add_next","add_to_queue"];
  const choices=[
    [modes[0],"play_circle","Reproducir ahora","Sustituye la cola actual"],
    [modes[1],"queue_play_next","Añadir al principio de la cola","Se reproducirá después de la pista actual"],
    [modes[2],"playlist_add","Añadir al final de la cola",`Se añadirá al final en ${zone.display_name}`]
  ];
  popover.innerHTML=`<div class="playback-popover-head"><small>Reproducir en ${esc(zone.display_name)}</small><strong>${esc(title)}</strong></div><div class="playback-action-list">${choices.map(([mode,glyph,label,detail])=>`<button role="menuitem" data-play-mode="${mode}" data-playback-kind="${esc(kind)}" data-result-id="${esc(resultId||"")}" data-playlist-id="${esc(playlistId||"")}" data-track-id="${esc(trackId||"")}">${icon(glyph)}<span><strong>${esc(label)}</strong><small>${esc(detail)}</small></span></button>`).join("")}</div>`;
  popover.hidden=false;
  try{popover.showPopover();}catch{}
  positionPlaybackActions(anchor);
  popover.querySelector("button")?.focus();
}
async function executePlaybackAction(button) {
  const zone = syncActiveZone();
  if (!zone) throw new Error("No hay una zona activa");
  const kind=button.dataset.playbackKind;
  if(kind==="playlist")await api(`/api/playlists/${encodeURIComponent(button.dataset.playlistId)}/play`,{method:"POST",body:JSON.stringify({zone_id:zone.zone_id,mode:button.dataset.playMode})});
  else if(kind==="playlist-track")await api(`/api/playlists/${encodeURIComponent(button.dataset.playlistId)}/tracks/${encodeURIComponent(button.dataset.trackId)}/play`,{method:"POST",body:JSON.stringify({zone_id:zone.zone_id,mode:button.dataset.playMode})});
  else await api(`/api/roon/media/${encodeURIComponent(button.dataset.resultId)}/play`, { method:"POST", body:JSON.stringify({ zone_id:zone.zone_id, mode:button.dataset.playMode }) });
  closePlaybackActions();
  notifyAction(["append","add_to_queue"].includes(button.dataset.playMode) ? "playback_append" : ["play_next","add_next"].includes(button.dataset.playMode) ? "playback_next" : "playback_now",{zone:zone.display_name,radio:false});
  if(kind==="playlist")await loadPlaylists();
  await refreshZoneContext();
}

function sortPlaylistItems(source) { const items=source.slice();if(state.playlistSort==="alphabetical")return items.sort((a,b)=>a.name.localeCompare(b.name,"es",{sensitivity:"base"}));return items.sort((a,b)=>{const aTime=a.last_played_at?Date.parse(a.last_played_at):0;const bTime=b.last_played_at?Date.parse(b.last_played_at):0;return bTime-aTime||a.name.localeCompare(b.name,"es",{sensitivity:"base"})}); }
function sortedPlaylists() { return sortPlaylistItems(state.playlists); }
function renderPlaylists() { const items=sortedPlaylists();const temporary=sortPlaylistItems(state.temporaryPlaylists);$("#playlist-sort").value=state.playlistSort;$("#playlist-summary").textContent=items.length===1?"1 playlist en tu colección.":`${items.length} playlists en tu colección.`;$("#playlist-grid").innerHTML=items.length?items.map(playlistCard).join(""):empty("Aún no hay playlists","Crea una colección y añade música desde cualquier búsqueda.","playlist_add");$("#temporary-playlist-summary").textContent=temporary.length===1?"1 playlist temporal activa.":`${temporary.length} playlists temporales activas.`;$("#temporary-playlist-grid").innerHTML=temporary.length?temporary.map(playlistCard).join(""):empty("No hay playlists temporales","Los asistentes todavía no han creado listas de trabajo activas.","schedule"); }
async function loadPlaylists() { const [payload,temporaryPayload]=await Promise.all([api("/api/playlists?scope=saved&limit=100&include_tracks=true&track_limit=16"),state.debugMode?api("/api/playlists?scope=temporary&limit=100&include_tracks=true&track_limit=16"):Promise.resolve({playlists:[]})]);state.playlists=payload.playlists||[];state.temporaryPlaylists=temporaryPayload.playlists||[];renderPlaylists(); }
function playlistCard(item) { const temporary=item.lifecycle?.type==='temporary';const played=temporary?`Caduca · ${fmtDate(item.lifecycle.expires_at)}`:item.last_played_at?`Última reproducción · ${fmtDate(item.last_played_at)}`:"Aún sin reproducir";const description=temporary?(item.lifecycle.intent||item.description||"Playlist temporal de trabajo"):(item.description||"Una playlist de roonIA");const duration=playlistDurationLabel(item);return `<button class="playlist-card ${temporary?'temporary':''}" data-playlist="${esc(item.playlist_id)}"><div class="playlist-cover">${playlistArtwork(item,item.name)}${temporary?'<span class="playlist-lifecycle-badge">Temporal</span>':''}<span class="playlist-open">${icon("arrow_forward")}</span></div><div class="playlist-card-body"><h3>${esc(item.name)}</h3><div class="playlist-meta"><span>${item.tracks_count || 0} canciones${duration?` · ${esc(duration)}`:""}</span><span>${esc(played)}</span></div><p title="${esc(description)}">${esc(description)}</p></div></button>`; }
function playlistTrackDuration(track){return Number(track.audio_metadata?.duration_seconds??track.identity?.duration_seconds)||0;}
function playlistTrackDebug(track){if(!state.debugMode)return "";const technical={track_id:track.track_id,query:track.query,roon_binding:track.roon_binding,identity:track.identity,resolution:track.resolution,audio_metadata:track.audio_metadata,user_metadata:track.user_metadata};return `<section class="track-technical" data-debug-only><span class="overline">Modo debug</span><h3>Datos técnicos</h3><pre>${esc(JSON.stringify(technical,null,2))}</pre></section>`;}
function playlistTrackRow(track,index,reordering){const duration=playlistTrackDuration(track);return `<div class="track-row playlist-track-row${reordering?' is-reordering':''}" data-track="${esc(track.track_id)}"${reordering?' draggable="true"':''}><span class="track-index">${reordering?icon("drag_indicator"):index+1}</span>${cover(track.image_key,track.title)}<button class="playlist-track-copy" data-open-playlist-track="${esc(track.track_id)}"${reordering?' disabled':''}><strong>${esc(track.title||track.query)}</strong><small>${esc(track.artist||"Artista desconocido")}</small></button><span class="track-album" title="${esc(track.album||"Álbum no disponible")}">${esc(track.album||"Álbum no disponible")}</span><span class="track-duration">${duration?fmtTime(duration):"—"}</span><span class="row-actions">${reordering?"":`<button class="track-action-button" data-track-actions="${esc(track.track_id)}" data-title="${esc(track.title||track.query||"Canción")}" title="Opciones de reproducción">${icon("play_arrow")}<span>Reproducir</span>${icon("arrow_drop_down")}</button><button class="icon-btn" data-remove-track title="Quitar">${icon("remove_circle")}</button>`}</span></div>`;}
function renderPlaylistDetail(playlist){const temporary=playlist.lifecycle?.type==='temporary';const reordering=state.playlistReorder?.playlistId===playlist.playlist_id;const lifecycle=temporary?`<div class="system-message warn"><strong>Playlist temporal</strong><br>${esc(playlist.lifecycle.intent||'Lista de trabajo creada por un asistente')} · Caduca ${esc(fmtDate(playlist.lifecycle.expires_at))}</div>`:'';const promote=temporary?`<button class="btn secondary" data-promote-playlist>${icon("bookmark_add")}Guardar como playlist</button>`:'';const duration=playlistDurationLabel(playlist);openModal(`${playlistModalHead(temporary?"Playlist temporal":"Playlist")}<div class="entity-hero"><div class="playlist-hero-artwork">${playlistArtwork(playlist,playlist.name)}</div><div class="entity-copy"><span class="overline">${playlist.tracks_count} canciones${duration?` · ${esc(duration)}`:""}</span><h2>${esc(playlist.name)}</h2><p>${esc(playlist.description||"Sin descripción")}</p>${lifecycle}<div class="inline-actions"><span class="split-action"><button class="btn primary" data-play-playlist="play_now">${icon("play_arrow")}Reproducir</button><button class="btn primary split-arrow" data-playlist-actions title="Opciones de reproducción" aria-label="Opciones de reproducción">${icon("arrow_drop_down")}</button></span>${promote}<button class="btn secondary" data-edit-playlist>${icon("edit")}Editar</button><button class="btn danger" data-delete-playlist>${icon("delete")}Eliminar</button></div></div></div><div class="modal-section"><div class="panel-head"><h3>Canciones</h3><div class="playlist-track-tools"><button class="btn secondary small" data-add-track${reordering?' disabled':''}>${icon("add")}Añadir canción</button>${reordering?`<button class="btn secondary small" data-cancel-reorder>Cancelar</button><button class="btn primary small" data-save-reorder>${icon("save")}Guardar orden</button>`:`<button class="btn secondary small" data-toggle-reorder${playlist.tracks?.length>1?'':' disabled'}>${icon("swap_vert")}Reordenar</button>`}</div></div>${playlist.tracks?.length?`<div class="track-list playlist-track-list${reordering?' is-reordering':''}">${playlist.tracks.map((track,index)=>playlistTrackRow(track,index,reordering)).join("")}</div>`:empty("Playlist vacía","Añade música desde la búsqueda o escribe una consulta.","playlist_add")}</div>`);}
async function openPlaylist(id) { state.modalSearch=null;state.playlistReorder=null;const playlist=await api(`/api/playlists/${encodeURIComponent(id)}?include_tracks=true&limit=500`);state.selectedPlaylist=playlist;renderPlaylistDetail(playlist); }
function renderPlaylistTrackDetail(track,item=null){const audio=track.audio_metadata||{};const merged={...(item||{}),...audio};const title=track.title||audio.title||item?.title||track.query;const artist=track.artist||audio.artist||item?.artist||item?.subtitle||"Artista desconocido";const album=track.album||audio.album||item?.album||"Álbum no disponible";const duration=Number(audio.duration_seconds??track.identity?.duration_seconds??item?.duration_seconds)||0;const source=String(item?.source||audio.source||track.identity?.source||"").trim();const year=audio.release_year||track.identity?.release_year||item?.release_year;const number=audio.track_number||track.identity?.track_number||item?.track_number;const imageKey=track.image_key||audio.image_key||item?.image_key;const facts=[["Artista",artist],["Álbum",album],["Duración",duration?fmtTime(duration):"No disponible"],["Disponibilidad",source&&source!=="unknown"?source.toUpperCase():"Roon"],["Género",merged.genre],["Compositor",merged.composer],["Año",year],["Pista",number]].filter(([,value])=>value);openContextModal(`<div class="modal-head"><div class="entity-head-nav"><button class="icon-btn" data-back-playlist title="Volver a la playlist">${icon("arrow_back")}</button><div><span class="overline">Canción de ${esc(state.selectedPlaylist?.name||"playlist")}</span><h2>${esc(title)}</h2></div></div><button class="icon-btn" data-close-context>${icon("close")}</button></div><div class="entity-hero playlist-track-hero">${cover(imageKey,title,"","track")}<div class="entity-copy"><span class="overline">${esc(source&&source!=="unknown"?`Disponible en ${source}`:"Disponible en Roon")}</span><h2>${esc(title)}</h2><p>${esc(artist)} · ${esc(album)}</p><div class="inline-actions"><button class="track-action-button detail-play-button" data-track-actions="${esc(track.track_id)}" data-title="${esc(title)}">${icon("play_arrow")}<span>Reproducir</span>${icon("arrow_drop_down")}</button></div></div></div><div class="track-facts">${facts.map(([label,value])=>`<div><small>${esc(label)}</small><strong>${esc(value)}</strong></div>`).join("")}</div>${playlistTrackDebug(track)}`);}
function playlistTrackMatches(track,item){const norm=(value)=>String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim();const title=norm(track.title||track.audio_metadata?.title);const artist=norm(track.artist||track.audio_metadata?.artist);const candidateTitle=norm(item?.title);const candidateArtist=norm(item?.artist||item?.subtitle);return Boolean(title&&candidateTitle===title&&(!artist||candidateArtist.includes(artist)||artist.includes(candidateArtist)));}
async function openPlaylistTrack(trackId){const track=state.selectedPlaylist?.tracks?.find((item)=>item.track_id===trackId);if(!track)throw new Error("No se encontró la canción de la playlist");renderPlaylistTrackDetail(track);try{const zone=syncActiveZone();const search=await api("/api/roon/media/search",{method:"POST",body:JSON.stringify({query:track.query||[track.title,track.artist,track.album].filter(Boolean).join(" "),types:["track"],count:25,zone_id:zone?.zone_id,source_preference:"streaming_first"})});const resultId=search.recommended_result_id||search.results?.[0]?.result_id;if(resultId){const item=await api(`/api/roon/media/${encodeURIComponent(resultId)}`);if(playlistTrackMatches(track,item)&&$("#context-modal").open&&$(`[data-track-actions="${CSS.escape(trackId)}"]`,$("#context-modal")))renderPlaylistTrackDetail(track,item);}}catch(error){if(state.debugMode&&$("#context-modal").open)toast(`La ficha muestra los metadatos guardados: ${error.message}`,"info");}}
function playlistForm(playlist = null, { context = false } = {}) { const preview=playlist?playlistArtwork(playlist,playlist.name):`<div class="cover-fallback">${icon("queue_music")}</div>`;const render=context?openContextModal:openModal;render(`${modalHead(playlist ? "Editar playlist" : "Nueva colección", playlist ? "Personaliza tu playlist" : "Crea una playlist")}<form id="playlist-form" class="modal-grid" data-parent="${context?'playlist':''}"><div class="playlist-cover-editor full"><div id="playlist-cover-preview" class="playlist-cover-preview">${preview}</div><div><span class="overline">Carátula</span><p>Sin imagen personalizada se genera un mosaico animado de 2×2, 3×3 o 4×4 con hasta 16 carátulas.</p><label class="btn secondary upload-cover-button">${icon("upload")}Subir imagen<input id="playlist-cover-file" type="file" accept="image/jpeg,image/png,image/webp" hidden></label>${playlist?.cover_image_key?.startsWith("custom:") ? `<button type="button" class="btn text danger-text" data-remove-custom-cover data-playlist-id="${esc(playlist.playlist_id)}">Volver al collage automático</button>` : ""}</div></div><label class="full">Nombre<input name="name" required maxlength="100" value="${esc(playlist?.name || "")}"></label><label class="full">Descripción<textarea name="description" maxlength="300">${esc(playlist?.description || "")}</textarea></label><input type="hidden" name="playlist_id" value="${esc(playlist?.playlist_id || "")}"><div class="modal-actions full"><button type="button" class="btn secondary" ${context?'data-close-context':'data-close'}>Cancelar</button><button class="btn primary" type="submit">${playlist ? "Guardar cambios" : "Crear playlist"}</button></div></form>`); }
...
function renderUpdateStatus(system){
  ...
  const channel=system.installed_channel||'stable';
  ...
  renderAvailableUpdateNotice(hasAvailable?{version:status.latest_version,build:status.latest_build,channel:status.channel}:null);
  if(hasAvailable){
    $("#available-version").textContent=`v${status.latest_version}${status.channel==='beta'?' (beta)':''}`;
...
async function loadSystem(){const [settings,system,dashboard]=await Promise.all([api('/api/admin/settings'),api('/api/admin/system'),api('/api/dashboard')]);state.installedChannel=system.installed_channel||'stable';renderConnection(dashboard.status);applyDebugMode(system.debug_mode);renderPortalVersion(system.version,state.installedChannel);$("#system-api-port").value=settings.api_port;$("#system-portal-port").value=settings.portal_port;const bridgeUrl=$("#system-bridge-url");const portalUrl=$("#system-portal-url");if(bridgeUrl)bridgeUrl.value=settings.public_base_url||'';if(portalUrl)portalUrl.value=settings.portal_base_url||'';$("#system-allow-beta").checked=system.allow_beta_updates===true;$("#system-auto-update-checks").checked=system.automatic_update_checks===true;$("#system-debug-mode").checked=state.debugMode;$("#system-temporary-playlist-expiry-days").value=system.temporary_playlist_expiry_days||7;$("#service-addresses").innerHTML=(system.addresses||[]).map((a)=>`<div class="address-row"><span>Acceso local · ${esc(a.address)}</span><code>${esc(a.portal_url)}</code></div>`).join('');$("#settings-grid").innerHTML=[[dashboard.status.core_connected?'Conectado':'Desconectado','Roon Core'],[dashboard.status.transport_ready?'Disponible':'No disponible','Control de reproducción'],[dashboard.status.browse_ready?'Disponible':'No disponible','Navegación musical'],[dashboard.counts.zones,'Zonas detectadas']].map(([v,l])=>`<div class="setting-row"><span>${esc(l)}</span><strong>${esc(v)}</strong></div>`).join('');renderDebugSystemDetails(settings,system);renderUpdateStatus(system);}
...
window.addEventListener('resize',()=>{scheduleFeaturedTitleFit();if(miniOutputPopoverOpen())positionMiniOutputPopover();});
...
$("#search-results").addEventListener('click',(e)=>{const actions=e.target.closest('[data-play-actions]');if(!actions)return;e.preventDefault();e.stopImmediatePropagation();openPlaybackActions(actions,{kind:"media",resultId:actions.dataset.playActions,title:actions.dataset.title});});
...
$("#new-playlist").addEventListener('click',()=>playlistForm());[$("#playlist-grid"),$("#temporary-playlist-grid")].forEach((grid)=>grid.addEventListener('click',(e)=>{const card=e.target.closest('[data-playlist]');if(card)openPlaylist(card.dataset.playlist).catch(err=>toast(err.message,'error'))}));
...
$("#home-hero").addEventListener('click',(e)=>{const preview=e.target.closest('[data-preview-zone]');if(preview){beginHomeZonePreview(preview.dataset.previewZone);return}const command=e.target.closest('[data-zone-command="playpause"]');if(command&&state.homePreviewZoneId)scheduleHomePreviewReturn(state.homePreviewZoneId);const zone=e.target.closest('[data-activate-zone]');if(zone)setActiveZone(zone.dataset.activateZone);});
...
$(".home-history").addEventListener('click',(e)=>{const more=e.target.closest('[data-history-more]');if(more){loadMoreHomeHistory(more.dataset.historyMore).catch(err=>toast(err.message,"error"));return}const row=e.target.closest('[data-home-history]');if(!row)return;const entry=[...state.homeHistory.play,...state.homeHistory.search].find((item)=>item.history_id===row.dataset.homeHistory);if(!entry)return;if(entry.event_type==="search"&&entry.query){state.musicTab="search";navigate("music").then(()=>{$("#music-query").value=entry.query;return searchMusic(entry.query);}).catch(err=>toast(err.message,"error"));return}if(entry.result_id)openMedia(entry.result_id).catch(err=>toast(err.message,"error"));});
...
$("#save-playlist-preferences").addEventListener('click',()=>savePlaylistPreferences().catch(err=>notifyError(err,'guardar la caducidad de las playlists temporales')));
...
document.addEventListener('click',(e)=>{const actions=e.target.closest('[data-track-actions]');if(!actions)return;e.preventDefault();e.stopPropagation();openPlaybackActions(actions,{kind:"playlist-track",playlistId:state.selectedPlaylist?.playlist_id,trackId:actions.dataset.trackActions,title:actions.dataset.title});});
$("#playback-actions-popover").addEventListener('click',(e)=>{const action=e.target.closest('[data-play-mode]');if(action){e.stopPropagation();action.disabled=true;executePlaybackAction(action).catch((error)=>{action.disabled=false;toast(error.message,'error')});}});
document.addEventListener('click',(e)=>{if(!e.target.closest('#playback-actions-popover,[data-play-actions],[data-track-actions],[data-playlist-actions]'))closePlaybackActions();});
document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&!$("#playback-actions-popover").hidden){e.preventDefault();closePlaybackActions();}});
window.addEventListener('resize',closePlaybackActions);window.addEventListener('scroll',closePlaybackActions,true);
...
$("#modal-content").addEventListener('click',(e)=>{
  ...
  if(e.target.closest('[data-edit-playlist]'))playlistForm(state.selectedPlaylist,{context:true});
  if(e.target.closest('[data-promote-playlist]')){const playlist=state.selectedPlaylist;api(`/api/playlists/${encodeURIComponent(playlist.playlist_id)}/promote`,{method:'POST',body:'{}'}).then(async(saved)=>{closeModal();notifyAction('playlist_updated',{name:saved.name});await loadPlaylists()}).catch(err=>notifyError(err,'guardar la playlist temporal'));}
  ...
  const detail=e.target.closest('[data-open-playlist-track]');if(detail){openPlaylistTrack(detail.dataset.openPlaylistTrack).catch(err=>notifyError(err,'abrir la ficha de la canción'));return;}
  const playlistActions=e.target.closest('[data-playlist-actions]');if(playlistActions){openPlaybackActions(playlistActions,{kind:'playlist',playlistId:state.selectedPlaylist.playlist_id,title:state.selectedPlaylist.name});return;}
  if(e.target.closest('[data-toggle-reorder]')){state.playlistReorder={playlistId:state.selectedPlaylist.playlist_id};renderPlaylistDetail(state.selectedPlaylist);return;}
  if(e.target.closest('[data-cancel-reorder]')){state.playlistReorder=null;renderPlaylistDetail(state.selectedPlaylist);return;}
  const saveOrder=e.target.closest('[data-save-reorder]');if(saveOrder){const trackIds=$$('.playlist-track-list [data-track]').map((item)=>item.dataset.track);busy(saveOrder,true);api(`/api/playlists/${encodeURIComponent(state.selectedPlaylist.playlist_id)}/tracks/reorder`,{method:'POST',body:JSON.stringify({track_ids:trackIds})}).then((playlist)=>{state.selectedPlaylist=playlist;state.playlistReorder=null;renderPlaylistDetail(playlist);notifyAction('playlist_updated',{name:playlist.name});return loadPlaylists()}).catch(err=>{busy(saveOrder,false);notifyError(err,'guardar el orden de la playlist')});return;}
  ...
  if(play){const zone=activeZone();const playlist=state.selectedPlaylist;if(!zone)return toast('No hay zona disponible','error');api(`/api/playlists/${encodeURIComponent(playlist.playlist_id)}/play`,{method:'POST',body:JSON.stringify({zone_id:zone.zone_id,mode:play.dataset.playPlaylist})}).then(async()=>{notifyAction('playlist_played',{name:playlist.name,zone:zone.display_name});await loadPlaylists()}).catch(err=>notifyError(err,'reproducir la playlist'));}
});
$("#modal-content").addEventListener('dragstart',(e)=>{const row=e.target.closest('.playlist-track-row[draggable="true"]');if(!row)return;row.classList.add('is-dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',row.dataset.track);});
$("#modal-content").addEventListener('dragover',(e)=>{const list=e.target.closest('.playlist-track-list.is-reordering');const dragging=list?.querySelector('.is-dragging');if(!list||!dragging)return;e.preventDefault();const rows=[...list.querySelectorAll('.playlist-track-row:not(.is-dragging)')];const next=rows.find((row)=>e.clientY<row.getBoundingClientRect().top+row.offsetHeight/2);list.insertBefore(dragging,next||null);});
$("#modal-content").addEventListener('drop',(e)=>{if(e.target.closest('.playlist-track-list.is-reordering'))e.preventDefault();});
$("#modal-content").addEventListener('dragend',(e)=>{const row=e.target.closest('.playlist-track-row');row?.classList.remove('is-dragging');});
...
document.addEventListener('submit',(e)=>{if(e.target.id!=='playlist-form')return;e.preventDefault();e.stopImmediatePropagation();const form=e.target;const data=formDataObject(form);const resultId=form.dataset.result;const parent=form.dataset.parent;const submit=form.querySelector('[type=submit]');busy(submit,true);(async()=>{let playlist=await api(data.playlist_id?`/api/playlists/${encodeURIComponent(data.playlist_id)}`:'/api/playlists',{method:data.playlist_id?'PATCH':'POST',body:JSON.stringify({name:data.name,description:data.description})});const file=form.querySelector('#playlist-cover-file')?.files?.[0];if(file){const dataUrl=await fileDataUrl(file);playlist=await api(`/api/playlists/${encodeURIComponent(playlist.playlist_id)}/cover`,{method:'POST',body:JSON.stringify({data_url:dataUrl})})}if(resultId)await api(`/api/playlists/${encodeURIComponent(playlist.playlist_id)}/tracks/from-search-result`,{method:'POST',body:JSON.stringify({result_id:resultId})});if(parent==='playlist'){closeContextModal();await openPlaylist(playlist.playlist_id)}else closeModal();notifyAction(data.playlist_id?'playlist_updated':'playlist_created',{name:playlist.name});await loadPlaylists()})().catch(err=>{busy(submit,false);notifyError(err,data.playlist_id?'actualizar la playlist':'crear la playlist')});});
...
$("#mini-player").addEventListener('click',(e)=>{const quickStep=e.target.closest('[data-mini-quick-zone-step]');if(quickStep){quickStep.disabled=true;const direction=Number(quickStep.dataset.miniQuickZoneStep);changeMiniZoneStep(direction,`[data-mini-quick-zone-step="${direction}"]`).catch((err)=>notifyError(err,'ajustar rápidamente el volumen'));return;}const b=e.target.closest('[data-mini-command]');if(b){const z=activeZone();if(z)zoneCommand(z.zone_id,b.dataset.miniCommand).catch(err=>notifyError(err,'controlar la reproducción'))}if(e.target.closest('[data-mini-queue]')){const z=activeZone();if(z)openQueue(z.zone_id).catch(err=>notifyError(err,'abrir la cola'))}if(e.target.closest('[data-mini-output]'))openMiniOutputPopover();});
...
$("#mini-player").addEventListener('input',(e)=>{if(e.target.matches('[data-mini-seek]'))setRangeFill(e.target);if(e.target.matches('[data-mini-seek]')){state.playerScrubbing=true;const label=$("[data-progress-current]");if(label)label.textContent=fmtTime(e.target.value)}});
$("#mini-player").addEventListener('change',(e)=>{if(e.target.matches('[data-mini-seek]')){const zone=activeZone();if(!zone){state.playerScrubbing=false;return}const seconds=Number(e.target.value);const playing=zone.state==='playing';zone.now_playing={...(zone.now_playing||{}),seek_position:seconds};setMiniPlayerSeekClock(seconds,playing);state.playerPendingUpdates++;e.target.blur();api(`/api/roon/zones/${encodeURIComponent(zone.zone_id)}/seek`,{method:'POST',body:JSON.stringify({mode:'absolute',seconds})}).then(()=>notifyAction('playback_seeked',{zone:zone.display_name,position:fmtTime(seconds)})).catch(err=>{state.miniRenderSignature=null;notifyError(err,`cambiar la posición en ${zone.display_name}`)}).finally(()=>{state.playerScrubbing=false;state.playerPendingUpdates--;refreshMiniPlayerState()})}});
$("#mini-output-popover").addEventListener('pointerdown',(e)=>{if(e.target.matches('input'))state.playerControlPointer=true;});
$("#mini-output-popover").addEventListener('input',(e)=>{if(!e.target.matches('[data-mini-volume]'))return;setRangeFill(e.target);const value=$("[data-mini-volume-value]",e.target.closest('[data-mini-output-row]'));if(value)value.textContent=miniVolumeLabel(e.target.value);});
$("#mini-output-popover").addEventListener('change',(e)=>{if(e.target.matches('[data-mini-volume]'))changeMiniOutputAbsolute(e.target).catch((err)=>notifyError(err,'ajustar el volumen de la salida'));});
$("#mini-output-popover").addEventListener('click',(e)=>{const zoneStep=e.target.closest('[data-mini-zone-step]');const outputStep=e.target.closest('[data-mini-volume-step]');const zoneChoice=e.target.closest('[data-mini-select-zone]');if(zoneStep){zoneStep.disabled=true;changeMiniZoneStep(Number(zoneStep.dataset.miniZoneStep)).catch((err)=>notifyError(err,'ajustar el volumen del grupo'));return;}if(outputStep){outputStep.disabled=true;changeMiniOutputStep(outputStep.dataset.outputId,Number(outputStep.dataset.miniVolumeStep)).catch((err)=>notifyError(err,'ajustar el volumen de la salida'));return;}if(zoneChoice){setActiveZone(zoneChoice.dataset.miniSelectZone);renderMiniOutputPopover();positionMiniOutputPopover();requestAnimationFrame(()=>$("[data-mini-select-zone].active",$("#mini-output-popover"))?.focus());}});
document.addEventListener('click',(e)=>{const popover=$("#mini-output-popover");const path=typeof e.composedPath==='function'?e.composedPath():[];const startedInside=path.some((node)=>node===popover||node?.matches?.('[data-mini-output]'));if(!startedInside&&!e.target.closest('#mini-output-popover,[data-mini-output]'))closeMiniOutputPopover();});
document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&miniOutputPopoverOpen()){e.preventDefault();closeMiniOutputPopover();$("[data-mini-output]")?.focus();}});
