...
function renderHomePlayback(force=false) {
  ...
  if(playingZones.length&&state.homePreviewZoneId)clearHomePreview();
...
$("#mini-player").addEventListener('input',(e)=>{if(e.target.matches('[data-mini-seek],[data-mini-volume]'))setRangeFill(e.target);if(e.target.matches('[data-mini-seek]')){state.playerScrubbing=true;const label=$("[data-progress-current]");if(label)label.textContent=fmtTime(e.target.value)}if(e.target.matches('[data-mini-volume]'))e.target.nextElementSibling.value=e.target.value;});
