...
function renderIdleHomeHero(online){
  ...
  root.innerHTML=`<div class="home-idle-ambient"></div><div class="home-idle-shell"><div class="home-idle-copy"><span class="overline">Tus zonas están en pausa</span><h1>Elige dónde seguir escuchando</h1><p>Selecciona una carátula para recuperar sus controles.</p></div><div class="home-idle-cloud">${state.zones.map((zone,index)=>{const np=zone.now_playing||{};return `<button type="button" class="home-idle-zone" data-preview-zone="${esc(zone.zone_id)}" style="--float-delay:-${index*1.7}s;--float-duration:${9+(index%4)*1.8}s"><span class="home-idle-art" style="view-transition-name:${homeZoneTransitionName(zone.zone_id)}">${cover(np.image_key,np.line1)}</span><span class="home-idle-label"><strong>${esc(zone.display_name)}</strong><small>${esc(np.line1||"Nada en reproducción")}</small></span></button>`;}).join("")}</div></div><div class="featured-meta"><span>${online?esc(state.dashboard?.status?.core_name||"Roon Core"):"Core desconectado"}</span><span>${state.dashboard?.counts?.playlists||0} playlists · ${state.dashboard?.counts?.playlist_tracks||0} pistas</span></div>`;
}
