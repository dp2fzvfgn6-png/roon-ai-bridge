...
function homeZoneTransitionName(zoneId){const index=state.zones.findIndex((zone)=>zone.zone_id===zoneId);return `home-zone-cover-${Math.max(0,index)}`;}
...
function homeIdleImageRequestSize(cssPixels,pixelRatio=1){const density=Math.min(2,Math.max(1,Number(pixelRatio)||1));const required=Math.ceil(Math.max(1,cssPixels)*density);return Math.min(1920,Math.max(640,Math.ceil(required/128)*128));}
...
function initHomeIdleMosaic(root){
  ...
  zones.forEach((zone,index)=>{const cover=$(".home-idle-art .cover",zone);if(cover)cover.style.viewTransitionName=homeZoneTransitionName(zone.dataset.previewZone);zone.addEventListener("pointerenter",()=>setActive(index));zone.addEventListener("focus",()=>setActive(index));});
...
function beginHomeZonePreview(zoneId){
  ...
  runHomeTransition(()=>{state.activeZoneId=zoneId;localStorage.setItem("roonia.portal.active-zone",zoneId);state.homePreviewZoneId=zoneId;state.homePlaybackSignature=null;renderMiniPlayer();renderHomePlayback(true);});
