...
function playlistCard(item) { const played=item.last_played_at?`Última reproducción · ${fmtDate(item.last_played_at)}`:"Aún sin reproducir";const description=item.description||"Una playlist de roonIA";return `<button class="playlist-card" data-playlist="${esc(item.playlist_id)}"><div class="playlist-cover">${playlistArtwork(item,item.name)}<span class="playlist-open">${icon("arrow_forward")}</span></div><div class="playlist-card-body"><h3>${esc(item.name)}</h3><div class="playlist-meta"><span>${item.tracks_count || 0} canciones</span><span>${esc(played)}</span></div><p title="${esc(description)}">${esc(description)}</p></div></button>`; }
