...
async function searchMusic(query) {
  ...
  state.search={query:cleaned,results:[],groups:emptySearchGroups(),warnings:[],category_status:Object.fromEntries(SEARCH_CATEGORIES.map(({type})=>[type,"loading"])),category_errors:{},selection_required:true,recommended_result_id:null,best_match:null};renderSearchResults();
  await Promise.allSettled(SEARCH_CATEGORIES.map(async({type,count})=>{
    try{
      ...
      state.search.results.push(...(payload.results||[]).map((item)=>({...item,is_best_match:false,selection_required:true})));
      Object.keys(state.search.groups).forEach((key)=>state.search.groups[key].push(...(payload.groups?.[key]||[]).map((item)=>({...item,is_best_match:false,selection_required:true}))));
      state.search.warnings.push(...(payload.warnings||[]));state.search.category_status[type]="complete";
  ...
  if(generation!==state.searchGeneration)return;state.searchController=null;renderSearchResults();
...
function renderSearchPayload(payload, target, { expanded = {}, playlistId = null } = {}) {
  ...
  const sections = definitions.filter(([key]) => (groups[key].length||(!["ep","single_ep","single"].includes(key)&&(statuses?.[statusKey(key)]==="loading"||statuses?.[statusKey(key)]==="error"))) && !(playlistId && key === "playlist")).map(([key,title,moreLabel]) => {
    ...
    const status=statuses?.[statusKey(key)];if(status==="loading"&&!visible.length)return `<section class="result-section search-section-loading"><div class="result-heading"><div><h2>${title}</h2><span>Buscando…</span></div></div><div class="search-result-skeleton" aria-label="Buscando ${esc(title)}"><i></i><i></i><i></i></div></section>`;
  ...
  root.innerHTML = `${playlistId ? "" : `<div class="section-title compact"><div><span class="overline">Resultados para</span><h2>“${esc(payload.query)}”</h2></div>${loading?`<span class="status-tag">Cargando categorías</span>`:""}</div>`}${!payload.selection_required&&payload.recommended_result_id?bestResult(payload.best_match,playlistId):""}${sections}`;
}
