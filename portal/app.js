...
async function loadHome() {
  ...
  renderQuickActions(); renderActivity($("#home-activity"), dashboard.recent_actions || []); renderMiniPlayer();
}
...
function renderHomePlayback(force=false) {
  ...
  const online=Boolean(dashboard?.status?.core_connected);const featured=syncActiveZone();const np=featured?.now_playing||{};
  $("#home-hero").classList.remove("skeleton-block"); $("#home-hero").innerHTML = featured ? `<div class="featured-backdrop">${imageTag(featured.now_playing?.image_key,"",1200)}</div><div class="featured-cover">${cover(featured.now_playing?.image_key,np.line1)}</div><div class="featured-content"><span class="overline">Ahora suena · ${esc(featured.display_name)}</span><h1>${esc(np.line1 || "Música en Roon")}</h1><p class="featured-artist">${esc(np.line2 || "")}</p><p class="featured-album">${esc(np.line3 || "")}</p><div class="featured-controls" data-zone="${esc(featured.zone_id)}"><button class="icon-btn" data-zone-command="previous" title="Anterior">${icon("skip_previous")}</button><button class="play-main" data-zone-command="playpause" title="${featured.state === "playing" ? "Pausa" : "Reproducir"}">${icon(featured.state === "playing" ? "pause" : "play_arrow")}</button><button class="icon-btn" data-zone-command="next" title="Siguiente">${icon("skip_next")}</button><button class="btn text" data-go="playback">Abrir zona ${icon("arrow_forward")}</button></div></div><div class="featured-meta"><span>${online ? esc(dashboard?.status?.core_name || "Roon Core") : "Core desconectado"}</span><span>${dashboard?.counts?.playlists||0} playlists · ${dashboard?.counts?.playlist_tracks||0} pistas</span></div>` : `<div class="featured-content empty-feature"><span class="overline">${online ? "Roon está preparado" : "Core sin conexión"}</span><h1>${greeting()}</h1><p>Busca un álbum, artista o canción para empezar a escuchar.</p><button class="btn primary" data-quick="music:search">${icon("search")}Buscar música</button></div>`;
...
function renderHomeNow() {
  $("#home-now-playing").innerHTML = state.zones.length ? state.zones.map((zone) => { const np = zone.now_playing || {}; const playing = zone.state === "playing"; return `<article class="now-card" data-zone="${esc(zone.zone_id)}">${cover(np.image_key, np.line1)}<div class="now-info"><div class="now-zone"><i class="${playing ? "online" : ""}"></i>${esc(zone.display_name)}</div><h3>${esc(np.line1 || "En silencio")}</h3><p class="entity-byline">${entityByline(np.line2,np.line3,"Nada en reproducción")}</p></div><div class="now-controls"><button class="icon-btn" data-zone-command="playpause" title="${playing ? "Pausar" : "Reproducir"}">${icon(playing ? "pause" : "play_arrow")}</button><button class="icon-btn" data-queue title="Abrir cola">${icon("queue_music")}</button></div></article>`; }).join("") : empty("No hay zonas", "Roon todavía no ha publicado ninguna zona.", "speaker_group");
}
...
function groupResults(payload) { const groups = { artist:[], album:[], ep:[], single_ep:[], single:[], track:[], playlist:[] }; const source=payload?.groups||null; const seen=new Set(); Object.keys(groups).forEach((key)=>{const items=source?.[key]||(!source?(payload?.results||[]).filter((item)=>releaseGroup(item)===key):[]);groups[key]=items.filter((result)=>{const identity=[result.media_type,result.title,result.artist||result.subtitle||"",result.album||""].join("|").toLocaleLowerCase();if(seen.has(identity))return false;seen.add(identity);return true;});});return groups; }
...
function splitArtistNames(value) { return [...new Map(String(value||"").replace(/[()[\]]/g," ").split(/\s*(?:,|;|\/|·|\bfeat(?:uring)?\.?|\bft\.?|\bwith\b|\bcon\b|\bx\b)\s*/iu).map((name)=>name.trim()).filter(Boolean).map((name)=>[name.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),name])).values()]; }
...
function renderSearchPayload(payload, target, { expanded = {}, playlistId = null } = {}) {
  ...
  root.innerHTML = `${playlistId ? "" : `<div class="section-title compact"><div><span class="overline">Resultados para</span><h2>“${esc(payload.query)}”</h2></div>${payload.ambiguous ? `<span class="status-tag warn">Hay varias coincidencias cercanas</span>` : ""}</div>`}${bestResult(payload.best_match,playlistId)}${sections}`;
}
...
function playlistCard(item) { const played=item.last_played_at?`Última reproducción · ${fmtDate(item.last_played_at)}`:"Aún sin reproducir";const description=item.description||"Una playlist de roonIA";return `<button class="playlist-card" data-playlist="${esc(item.playlist_id)}"><div class="playlist-cover">${playlistArtwork(item,item.name)}<span class="playlist-open">${icon("arrow_forward")}</span></div><div class="playlist-card-body"><h3>${esc(item.name)}</h3><p title="${esc(description)}">${esc(description)}</p><div class="playlist-meta"><span>${item.tracks_count || 0} canciones</span><span>${esc(played)}</span></div></div></button>`; }
