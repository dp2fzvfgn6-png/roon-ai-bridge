...
function wrapper(initialView) {
  ...
  window.addEventListener("message",event=>{if(event.source!==frame.contentWindow)return;const msg=event.data;if(!msg||msg.jsonrpc!=="2.0"||!msg.id)return;
    if(msg.method==="tools/call"){const name=msg.params?.name;const args=msg.params?.arguments||{};let result;if(name==="roon_ui_navigate")result={structuredContent:{status:"completed",view:args.view,widget:fixture(args.view,args)}};else result={structuredContent:{status:"completed",summary:"Preview action completed.",data:{name,args}}};frame.contentWindow.postMessage({jsonrpc:"2.0",id:msg.id,result},"*");}
