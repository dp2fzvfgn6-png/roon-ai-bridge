...
export const WIDGET_V2_VERSION = "v15";
...
<style>
  ...
  *{box-sizing:border-box}html,body{margin:0;min-width:260px;background:transparent;color:var(--text)}
  ...
  .art{overflow:hidden;aspect-ratio:1;background:linear-gradient(145deg,#202824,#101311);border:1px solid var(--line)}.art img{display:block;width:100%;height:100%;object-fit:cover}.art.placeholder{display:grid;place-items:center;color:var(--green-light);font-size:10px;letter-spacing:.08em}
  ...
  .search-title{margin-bottom:13px}.best-match{display:grid;grid-template-columns:92px minmax(0,1fr);gap:15px;align-items:center;margin:0 0 18px;padding:12px;border:1px solid var(--green);background:linear-gradient(135deg,rgba(103,132,117,.18),var(--bg-soft))}.best-match .art{width:92px}.best-match h2{font-size:20px}.best-match p:last-child{margin:0;color:var(--text-soft);font-size:11px}.empty{margin:0;padding:26px 8px;color:var(--text-dim);font-size:12px;text-align:center}.warning{margin:16px 0 0;padding-top:10px;border-top:1px solid var(--line);color:var(--text-dim);font-size:9px}
...
<script type="module">
  ...
  function art(url,kind){return url?'<div class="'+(kind||"art")+'"><img src="'+attr(url)+'" alt="" loading="lazy"></div>':'<div class="'+(kind||"art")+' placeholder">roonIA</div>'}
  ...
  function card(item){const subtitle=[item.artist||item.album_artist||item.subtitle,item.release_year].filter(Boolean).join(" · ");return '<article class="card">'+art(item.image_url)+'<h3>'+esc(item.title)+'</h3><p>'+esc(subtitle)+'</p></article>'}
  function cards(items){return '<div class="cards">'+items.map(card).join("")+'</div>'}
  function row(item,index){const detail=[item.artist,item.album].filter(Boolean).join(" · ");return '<div class="row"><span class="index">'+esc(item.position||item.track_number||index+1)+'</span>'+art(item.image_url,"art cover")+'<span class="row-copy"><strong>'+esc(item.title)+'</strong><small>'+esc(detail)+'</small></span><span class="duration">'+time(item.duration_seconds)+'</span></div>'}
  ...
  function hero(item,kicker,description){const quality=item.quality&&item.quality.label?'<span class="quality">'+esc(item.quality.label)+'</span>':"";return '<section class="hero">'+art(item.image_url)+'<div class="hero-copy"><p class="overline">'+esc(kicker)+'</p><h1>'+esc(item.title)+'</h1><p class="subtitle">'+esc([item.artist||item.album_artist,item.album,item.release_year].filter(Boolean).join(" · "))+'</p>'+(description?'<p class="description">'+esc(description)+'</p>':"")+quality+'</div></section>'}
  ...
  function renderSearch(data){label.textContent="Resultados";const groups=data.groups||{},best=data.best_match;const bestHtml=best?'<section class="best-match">'+art(best.image_url)+'<div><p class="overline">Mejor resultado · '+esc(best.release_type&&best.release_type!=="unknown"?best.release_type:best.media_type)+'</p><h2>'+esc(best.title)+'</h2><p>'+esc(best.artist||best.album_artist||best.subtitle||"")+'</p></div></section>':"";const grouped=section("Artistas",groups.artist,cards)+section("Álbumes",groups.album,cards)+section("EPs",groups.ep,cards)+section("Singles / EPs",groups.single_ep,cards)+section("Singles",groups.single,cards)+section("Canciones",groups.track,rows)+section("Playlists",groups.playlist,cards);return '<div class="search-title"><p class="overline">Búsqueda</p><h2>'+esc(data.title)+'</h2></div>'+bestHtml+(grouped||'<p class="empty">No se encontraron resultados.</p>')+warning(data)}
  function renderArtist(data){label.textContent="Artista";return hero(data.artist||{},"Artista",null)+section("Canciones populares",data.popular_tracks,rows)+section("Álbumes",data.albums,cards)+section("EPs y singles",data.singles_eps,cards)+warning(data)}
