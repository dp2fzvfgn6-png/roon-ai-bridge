...
export function createPlaylistsRouter(context: ApiContext): Router {
  ...
  router.get("/playlists", (req, res, next) => {
    ...
    next(
      new ApiError("NOT_IMPLEMENTED", "Virtual playlists are not implemented in v0.3")
    );
  });
  ...
  router.post("/playlists", (req, res, next) => {
    ...
    next(
      new ApiError(
        "NOT_IMPLEMENTED",
        "Virtual playlist creation is not implemented in v0.3"
      )
    );
  });
  ...
  router.post("/playlists/:playlist_id/play", (req, res, next) => {
    ...
    next(
      new ApiError(
        "NOT_IMPLEMENTED",
        "Virtual playlist playback is not implemented in v0.3"
      )
    );
  });
